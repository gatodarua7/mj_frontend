<template>
  <v-main>
    <barra-superior
      ...
      subtitulo="Analisar Cadastro do Visitante"
            ...
            <v-row>
              ...
              >
                <v-file-input
                  ...
                  @change="verificarArquivoDiretoria"
                />
              </v-col>
              <v-col
                cols="12"
              ...
              >
                <v-btn
                  ...
                  @click="adicionarArquivoDiretoria"
                >
                  Incluir
                </v-btn>
              </v-col>
...
export default {
  ...
  data: () => ({
    breadcrumbs: [
      ...
      {
        text: 'Análise da Diretoria',
        to: '/visitante/diretoria',
        exact: true
      },
      {
        text: 'Manifestar',
  ...
  methods: {
    ...
    getHistoricoManifestacaoDiretoria(visitanteId, analiseId){
      visitanteService
        .historicoManifestacaoDiretoria(visitanteId, analiseId)
        .then(res => {
    ...
    getVisitante(id) {
      visitanteService
        ...
        .then(res => {
          ...
          this.getHistoricoManifestacaoDiretoria(this.visitante.id, this.visitante.usuario_permissao ? this.visitante.analise_diretoria : null)
          this.visitante.analise_diretoria &&
            this.visitante.usuario_permissao &&
              this.carregarAnalise(this.visitante.analise_diretoria)
          this.carregado = true
        })
        .catch(err => {
          this.displayErrors(err)
      ...
      if(this.arquivo){
        ...
        if (isImage || isPdf) {
          if(isImage && this.arquivo.size > 1*1024*1024){
            this.errorsArquivos.arquivo_temp = 'Tamanho da imagem não suportado. O arquivo deve ter no máximo 1MB'
            this.disableSubmitArquivo = true
          } else if(isPdf && this.arquivo.size > 15*1024*1024){
            this.errorsArquivos.arquivo_temp = 'Tamanho do arquivo não suportado. O arquivo deve ter no máximo 15MB'
            this.disableSubmitArquivo = true
          } else {
            this.errorsArquivos.arquivo_temp = null
            this.disableSubmitArquivo = false
          }
        } else {
          this.errorsArquivos.arquivo_temp = 'Formato de arquivo não suportado'
        }
      }
    },
    adicionarArquivoDiretoria() {
    ...
    carregarAnalise(analiseId){
      visitanteService
        .getManifestacaoDiretoria(analiseId)
        .then(res=>{
          this.analise = res.data
          this.arquivos = res.data.documentos
        })
    },

<template>
  <v-main>
    <barra-superior
      ...
      :breadcrumbs="$t('escolta.top.breadcrumbsCadastrar')"
    />
    <v-container fluid>
      <v-card>
        <v-card-text>
          <search-pedidos :object="escolta" />
          <v-card flat>
            <v-card-title>
...
export default {
  ...
  mounted() {
    const pedidos = this.$route.params.ids
    const list_pedidos = pedidos.split(',')
    this.escolta.pedidos_inclusao = list_pedidos.map(item=>{return {id: item}})
  },
  methods: {
    limpar() {
    ...
    salvar() {
      ...
      escoltaService
        .inserir(this.escolta)
        .then(()=>{
          this.$router.push('/escolta')

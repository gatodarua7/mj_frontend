...
export default {
  ...
  mounted() {
    this.getUnidades()
    let pedidoId = this.$route.params.pedidoId
    let analiseId = this.$route.params.analiseId
    this.carregarDados(pedidoId, analiseId)
  },
  methods: {
    async carregarDados(id, analise) {
      pedidoInclusaoService
        ...
        .then(res => {
          ...
          this.carregarHistorico(res.data.id, analise)
          this.carregado =  true
        })
        .catch(error => {
          this.displayErrors(error)
        })
      const response = await this.$api.get({resource: this.$endpoints.ANALISE_PEDIDO, id: analise})
      this.analiseCgin = response.data
      this.posicionamento = this.analiseCgin.posicionamento == 'FAVORAVEL'
    },
    async carregarHistorico(pedidoInclusaoId, analiseId) {
      const response = await this.$api.list({resource: this.$endpoints.ANALISE_PEDIDO, query: {pedido_inclusao: pedidoInclusaoId, analise: analiseId}})
      this.historico = response.data.results
    },
    salvar() {
      this.analiseCgin.posicionamento = this.posicionamento ? 'FAVORAVEL' : 'DESFAVORAVEL'
      const response = this.$api.update({resource: this.$endpoints.ANALISE_PEDIDO, id: this.analiseCgin.id, data: this.analiseCgin})
      response
        .then(() => {
          this.$router.back()

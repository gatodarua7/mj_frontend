<template>
  <v-main>
    ...
    <v-container fluid>
        ...
        >
          ...
          >
            <v-list-item three-line>
              <v-list-item-content>
                <div class="mb-2">
                  Esperando resposta do juízo federal corregedor
                </div>
                <div class="headline">
                  {{ totalRemetido }}
                </div>
        ...
        >
          <v-row>
            ...
            >
              ...
              >
                <v-card-title>
                  <span class="subtitle-1">Total de Parecer Favorável ou Desfavorável pela CGIN</span>
                </v-card-title>
                <v-card-text>
                  <chart
                    v-if="totalParecerCgin.datasets"
                    name="totalParecerCgin"
                    type="pie"
                    :data="totalParecerCgin"
                  />
                </v-card-text>
              </v-card>
            </v-col>
            ...
            >
              ...
              >
                <v-card-title>
                  <span class="subtitle-1">Total de Indicações da CGIN por Penitenciária Federal</span>
                </v-card-title>
                <v-card-text>
                  <chart
                    v-if="totalUnidadesCgin.datasets"
                    name="totalUnidadesCgin"
                    type="pie"
                    :data="totalUnidadesCgin"
                  />
                </v-card-text>
              </v-card>
            </v-col>
            ...
            >
              ...
              >
                <v-card-title>
                  <span
                    class="subtitle-1"
                  >Pedidos de Inclusão dos Últimos 12 meses - Concluídos / Arquivados / Deferido pelo juízo Corregedor/ Indeferido pelo juízo Corregedor/ Parecer Favorável da CGIN / Parecer Desfavorável da CGIN</span>
                </v-card-title>
                <v-card-text>
                  <div class="d-flex align-center">
                    ...
                    <div class="chartAreaWrapper">
                      <div class="chartTotalFasesMes">
                        <chart
                          v-if="totalFasesMes.datasets"
                          name="totalFasesMes"
                          type="bar"
                          :data="totalFasesMes"
                        />
      ...
      <estado-detail-dialog ref="estadoDetail" />
    </v-container>
  </v-main>
</template>
...
import EstadoDetailDialog from './components/EstadoDetailDialog.vue'
import Chart from '@/components/Chart.vue'

export default {
  components: { EstadoDetailDialog, Chart },
  data: () => ({
    totalPendente: 0,
    totalAnalise: 0,
    totalRemetido: 0,
    ...
    totalParecerCgin: {},
    totalUnidadesCgin: {},
    totalEstadoSolicitante: {},
    totalFasesMes: {},
  }),
  mounted() {
    this.getTotalFases()
    this.getTotalParecerCgin()
    this.getTotalUnidadesCgin()
    this.getTotalFasesMes()
    this.getTotalEstadoSolicitante()
  },
  methods: {
    getTotalFases() {
      pedidoInclusaoService
        .getTotalFases()
        .then(res => {
          this.totalPendente = res.data.pendente
          this.totalAnalise = res.data.em_analise
          this.totalRemetido = res.data.remetido
          this.totalArquivado = res.data.arquivado
        })
    },
    getTotalParecerCgin() {
      pedidoInclusaoService
        .getTotalParecerCgin()
        .then(res => {
          this.totalParecerCgin = res.data
        })
    },
    getTotalUnidadesCgin() {
      pedidoInclusaoService
        .getTotalUnidadesCgin()
        .then(res => {
          this.totalUnidadesCgin = res.data
        })
    },
    getTotalFasesMes() {
      pedidoInclusaoService
        .getTotalFasesMes()
        .then(res => {
          this.totalFasesMes = res.data
        })
    },
    getTotalEstadoSolicitante() {
      pedidoInclusaoService
        .getTotalEstadoSolicitante()
        .then(res => {
          this.totalEstadoSolicitante = res.data
        })
    },
    onEstadoHover(evt) {
      ...
      this.$refs.estadoDetail.open(evt.srcElement.parentElement.id, this.totalEstadoSolicitante[evt.srcElement.parentElement.id], evt.clientX, evt.clientY)
    },
    onClickRight() {
      let content = document.querySelector('.chartAreaWrapper')

<template>
  <div>
    <v-data-table
      :headers="headers"
      :items="object.telefones_funcionais"
      :footer-props="{
...
export default {
  ...
  methods: {
    ...
    editarItem(item) {
      this.telefoneIndex = this.object.telefones_funcionais.indexOf(item)
    ...
    async excluirItem(item) {
      if (
        ...
        const index = this.object.telefones_funcionais.indexOf(item)
        this.object.telefones_funcionais.splice(index, 1)
      }
    },
    getItem() {
      telefoneService
        .searchByPhoneWithoutPrivacy(this.phoneNumber)
        .then(response => {
          this.clearItem()
    ...
    checkPhoneAdded() {
      ...
      this.object.telefones_funcionais.map(item => {
        item.numero == this.phoneNumber &&
          telefonesAdicionados.push(item.numero)
      })
      if (telefonesAdicionados.length) {
        this.$store.dispatch('snackbar/setSnackbar', {
          text: 'Este telefone já foi adicionado ao cadastro',
          color: 'error',
          timeout: 3000
        })
        this.close()
    ...
    salvar() {
      ...
      response
        .then(res => {
          this.telefoneIndex > -1
            ? Object.assign(
                this.object.telefones_funcionais[this.telefoneIndex],
                res.data
              )
            : this.object.telefones_funcionais.push(res.data)
          this.close()
        })
        .catch(error => {
